...
import { perf_fa_create, perf_fa_login, perf_fa_logout, perf_fa_tvalid, perf_fb_insert, perf_fb_remove, perf_fb_update, perf_fc_get, perf_fu_search } from "backend_api_types/performance";
import { getDeferredPromise } from "backend_api_types/DeferredPromise";
/*
...
async function test_compare_get_posts(client, post_ids) {
    ...
    if (!client.token_hash)
        throw ("no token_hash");
    const res = await test_fetch(ENDPOINTS.combined_get, {
        user_id: client.user_id,
        token_hash: client.token_hash,
        posts: post_ids,
    });
