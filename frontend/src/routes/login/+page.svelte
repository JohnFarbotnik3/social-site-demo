<script>
	...
	async function onlogin() {
		...
		const { success, message } = await API.account_login(username, password);
		if(success) base_goto("/profile"); else submit_error = message;
	}
	...
	async function onsubmit() {
		...
		const { success, message } = await API.account_create(username, password, nickname);
		if(success) base_goto("/profile"); else submit_error = message;
	}
