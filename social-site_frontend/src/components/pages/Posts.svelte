<script>
	...
	function is_content_safe(content) {
		// filter out tags that are considered safe.
		let str = content;
		const allowed_tags = [
			"<br>", "</br>",
			"<b>", "</b>",
		...
		for(const tag of allowed_tags) str = str.replaceAll(tag, "");
		// check if content contains any unsafe stuff.
		if(str.includes("<") || str.includes(">")) {
			console.log("unsafe post content");
			console.log(content);
			console.log(str);
			return false;
		}
		...
		<div class="posts_area">
			{#each posts as post}
				<div class="post">
					...
					<div class="body">
						{#if is_content_safe(post.content)}
							{@html post.content}
						{:else}
							{post.content}
						{/if}
